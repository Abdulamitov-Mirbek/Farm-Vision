...
import {
  ...
  Save, Edit2, Award, Target, Clock, Users, DollarSign, Leaf,
    ...
    <div className={`p-6 transition-colors duration-300 ${themeClasses.page}`}>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className={`text-2xl font-bold ${themeClasses.text.primary} flex items-center`}>
              <User className={`mr-2 ${
                theme === 'dark' ? 'text-green-400' : 'text-green-600'
              }`} size={24} />
            ...
            <p className={`mt-1 ${themeClasses.text.secondary}`}>
              {language === 'ru' ? 'Личная информация и статистика' :
               language === 'en' ? 'Personal information and statistics' :
          ...
          <div className="flex items-center space-x-3 mt-4 md:mt-0">
            ...
            >
              ...
              {language === 'ru' ? 'Выйти' : language === 'en' ? 'Logout' : 'Чыгуу'}
            </button>
          </div>
        </div>
          ...
          <div className="lg:col-span-1">
            <div className={`p-6 rounded-xl border ${themeClasses.card} ${themeClasses.shadow.card}`}>
              ...
              <div className="relative group mb-4 inline-block">
                <div className={`w-32 h-32 rounded-2xl bg-gradient-to-br from-green-400 to-green-600 flex items-center justify-center text-white text-5xl shadow-lg mx-auto overflow-hidden ${
                ...
                <label className={`absolute bottom-0 right-0 w-8 h-8 rounded-lg shadow-md flex items-center justify-center cursor-pointer transition-colors border ${
                  theme === 'dark'
              ...
              <div className={`mt-4 pt-4 border-t ${themeClasses.divider}`}>
                <div className={`flex items-center justify-center text-sm mb-2 ${themeClasses.text.secondary}`}>
                  <MapPin size={14} className="mr-1" />
                  {userData.location?.city || 'Город не указан'}, {userData.location?.country || 'Страна не указана'}
                </div>
                <div className={`flex items-center justify-center text-sm ${themeClasses.text.secondary}`}>
                  <Calendar size={14} className="mr-1" />
                  {language === 'ru' ? 'Регистрация' : language === 'en' ? 'Joined' : 'Катталган'}: {formatDate(userData.joinDate)}
                </div>
              </div>
              ...
              <div className="mt-4 flex justify-center space-x-4">
                <div className="text-center">
                  ...
                  <p className={`text-xs ${themeClasses.text.muted}`}>
                    {language === 'ru' ? 'Полей' : language === 'en' ? 'Fields' : 'Талаалар'}
                  </p>
                </div>
                <div className="text-center">
                  <p className={`text-xl font-bold ${themeClasses.text.primary}`}>
                  ...
                  <p className={`text-xs ${themeClasses.text.muted}`}>
                    {language === 'ru' ? 'Задач' : language === 'en' ? 'Tasks' : 'Милдеттер'}
                  </p>
                </div>
                <div className="text-center">
                  <p className={`text-xl font-bold ${themeClasses.text.primary}`}>
                  ...
                  <p className={`text-xs ${themeClasses.text.muted}`}>
                    {language === 'ru' ? 'Достижения' : language === 'en' ? 'Achievements' : 'Жетишкендиктер'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className={`p-6 rounded-xl border ${themeClasses.card} ${themeClasses.shadow.card}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <p className={`text-sm mb-1 ${themeClasses.text.secondary}`}>
                    {language === 'ru' ? 'Имя' : language === 'en' ? 'Name' : 'Аты'}
                  </p>
                ...
                <div>
                  <p className={`text-sm mb-1 ${themeClasses.text.secondary}`}>Email</p>
                  <p className={`font-medium ${themeClasses.text.primary}`}>
                    {userData.email || '—'}
                  </p>
                </div>
                <div>
                  <p className={`text-sm mb-1 ${themeClasses.text.secondary}`}>
                    {language === 'ru' ? 'Телефон' : language === 'en' ? 'Phone' : 'Телефон'}
                  </p>
                  <p className={`font-medium ${themeClasses.text.primary}`}>
                    {userData.phone || '—'}
                  </p>
                ...
                <div>
                  <p className={`text-sm mb-1 ${themeClasses.text.secondary}`}>
                    {language === 'ru' ? 'Ферма' : language === 'en' ? 'Farm' : 'Ферма'}
                  </p>
                  <p className={`font-medium ${themeClasses.text.primary}`}>
                    {userData.farmName || '—'}
                  </p>
              ...
              {userData.bio && (
                <div className="mt-4">
                  <p className={`text-sm mb-1 ${themeClasses.text.secondary}`}>
                    {language === 'ru' ? 'О себе' : language === 'en' ? 'Bio' : 'Өзүм жөнүндө'}
                  </p>
                  <p className={`p-3 rounded-lg ${themeClasses.bio.bg} ${themeClasses.bio.text}`}>
                    {userData.bio}
                  </p>
                </div>
              )}
            ...
            <div className={`p-6 rounded-xl border ${themeClasses.card} ${themeClasses.shadow.card}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className={`text-center p-4 rounded-xl ${themeClasses.statCard.bg}`}>
                  <p className={`text-2xl font-bold ${themeClasses.statCard.value}`}>
                    {userData.stats.totalFields || 0}
                  </p>
        ...
        {isEditing && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
            <div className={`w-full max-w-2xl rounded-xl border ${themeClasses.card} ${themeClasses.shadow.card} p-6`}>
              <h3 className={`text-xl font-semibold mb-4 ${themeClasses.text.primary}`}>
                {language === 'ru' ? '✏️ Редактировать профиль' : 
                 language === 'en' ? '✏️ Edit Profile' : 
                 '✏️ Профилди оңдоо'}
              </h3>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className={`block text-sm font-medium mb-1 ${themeClasses.text.secondary}`}>
                    {language === 'ru' ? 'Название фермы' : language === 'en' ? 'Farm Name' : 'Ферманын аты'}
                  </label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className={`block text-sm font-medium mb-1 ${themeClasses.text.secondary}`}>
                      {language === 'ru' ? 'Город' : language === 'en' ? 'City' : 'Шаар'}
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className={`block text-sm font-medium mb-1 ${themeClasses.text.secondary}`}>
                      {language === 'ru' ? 'Страна' : language === 'en' ? 'Country' : 'Өлкө'}
                    </label>
                    <input
                      type="text"
                ...
                <div>
                  ...
                  <textarea
                    rows={4}
                    className={`w-full px-4 py-2 rounded-lg border transition-colors ${themeClasses.input}`}
                    placeholder={language === 'ru' ? 'О себе...' : language === 'en' ? 'About...' : 'Өзүм жөнүндө...'}
                    value={editForm.bio}
